...
private:
    ...
    void upheap(size_type index) {
        ...
        if (comp(c.at(parentIndex), c.at(index))){
            std::swap(c.at(parentIndex), c.at(index));
            upheap(parentIndex);
        }
        return;
    }
    ...
    void downheap(size_type index) {
        ...
        size_type leftChild = left_child(index);
        size_type rightChild = right_child(index);
        size_type maxPriorityChild = leftChild;
        if(rightChild < c.size() && comp(c.at(leftChild), c.at(rightChild))){
            maxPriorityChild = rightChild;
        }
        if(comp(c.at(index), c.at(maxPriorityChild))){
            std::swap(c.at(index), c.at(maxPriorityChild));
            downheap(maxPriorityChild);
        }
        return;
    }
